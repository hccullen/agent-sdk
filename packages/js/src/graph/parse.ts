...
export type WorkflowHandler = (
  state: AnyState,
) => Promise<WorkflowHandlerResult | Partial<AnyState>>;
...
export function parseWorkflowDefinition(
  ...
  if (!d.document || typeof d.document !== "object") {
    throw new Error(
      "[DeclarativeGraph] Missing or invalid required field: document.",
    );
  }
  const doc = d.document as Record<string, unknown>;
  if (!doc.name || typeof doc.name !== "string") {
    throw new Error(
      "[DeclarativeGraph] document.name is required and must be a string.",
    );
  }
  if (!doc.version || typeof doc.version !== "string") {
    throw new Error(
      "[DeclarativeGraph] document.version is required and must be a string.",
    );
  }

  if (!Array.isArray(d.nodes) || d.nodes.length === 0) {
    throw new Error(
      "[DeclarativeGraph] nodes is required and must be a non-empty array.",
    );
  }
  if (!Array.isArray(d.edges)) {
    throw new Error(
      "[DeclarativeGraph] edges is required and must be an array.",
    );
  }
  ...
  for (const node of d.nodes) {
    ...
    if (!n.type || typeof n.type !== "string" || !validNodeTypes.has(n.type)) {
      throw new Error(
        `[DeclarativeGraph] Node "${n.id}" has invalid type. Must be one of: agent_call, switch, set_state, http_call, interrupt, wait, parallel, callback, end.`,
      );
    }
    if (n.type !== "end" && (!n.config || typeof n.config !== "object")) {
      throw new Error(
        `[DeclarativeGraph] Node "${n.id}" must have a config object.`,
      );
    }
    if (n.type === "callback") {
      ...
      if (!cfg?.handler || typeof cfg.handler !== "string") {
        throw new Error(
          `[DeclarativeGraph] Callback node "${n.id}" must have a string handler.`,
        );
      }
    }
  }
  ...
  for (const edge of d.edges) {
    ...
    if (
      ...
      throw new Error(
        "[DeclarativeGraph] Each edge must have string source and target.",
      );
    }
    if (e.source === "__start__") hasStart = true;
    if (!nodeIds.has(e.target)) {
      throw new Error(
        `[DeclarativeGraph] Edge target "${e.target}" does not match any node id.`,
      );
    }
  }
  ...
  if (!hasStart) {
    throw new Error(
      '[DeclarativeGraph] No edge with source "__start__" found.',
    );
  }
  ...
  for (const node of d.nodes) {
    ...
    if (n.id === "__end__" && n.type !== "end") {
      throw new Error(
        `[DeclarativeGraph] Node "__end__" must have type "end".`,
      );
    }
  ...
  if (!hasEnd) {
    throw new Error(
      '[DeclarativeGraph] No node with id "__end__" and type "end" found.',
    );
  }
